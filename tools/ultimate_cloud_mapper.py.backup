...
class UltimateCloudMapper:
    ...
    def __init__(self):
        ...
        self.synonyms = {
            ...
            "backend_bucket": "storage", "url_map": "loadbalancing", "https_proxy": "loadbalancing",
            "forwarding_rule": "loadbalancing", "ssl_certificate": "security", "security_policy": "security"
        }
        
        # GCP-specific service mappings for better accuracy
        self.gcp_service_mappings = {
            "storage_bucket": "storage",
            "compute_security_policy": "security", 
            "compute_backend_bucket": "storage",
            "compute_url_map": "network", 
            "compute_target_https_proxy": "network",
            "compute_global_forwarding_rule": "network", 
            "compute_managed_ssl_certificate": "security",
        ...
        self.common_mappings = {
            ...
            "google_compute_security_policy": "securitypolicy", "google_compute_backend_bucket": "storage",
            "google_compute_url_map": "loadbalancing", "google_compute_target_https_proxy": "loadbalancing",
            "google_compute_global_forwarding_rule": "loadbalancing", "google_compute_managed_ssl_certificate": "security",
    ...
    def get_icon(self, tf_type, debug=False):
        ...
        # Strategy 1.5: GCP-specific mappings for better accuracy
        if prefix == "google" and len(parts) > 2:
            gcp_service = "_".join(parts[1:]).lower()
            if gcp_service in self.gcp_service_mappings:
                mapped_name = self.gcp_service_mappings[gcp_service]
                icons = self.index_all_icons(prefix)
                # Try to find matching icons in GCP categories
                for category_name, icon_class in icons.items():
                    if mapped_name in category_name or category_name in mapped_name:
                        if debug:
                            print(f"[UltimateMapper] GCP-specific mapping: {gcp_service} -> {category_name}")
                        return icon_class
                ...
                try:
                    ...
                    # GCP common categories
                    gcp_categories = ["storage", "compute", "network", "security", "database", "analytics"]
                    for category in gcp_categories:
                        if mapped_name in category and hasattr(provider_mod, category):
                            cat_mod = getattr(provider_mod, category)
                            for class_name in dir(cat_mod):
                                if not class_name.startswith("__"):
                                    icon_cls = getattr(cat_mod, class_name)
                                    if hasattr(icon_cls, '__bases__') and any(base.__name__ == 'Node' for base in icon_cls.__bases__):
                                        if debug:
                                            print(f"[UltimateMapper] GCP category match: {category}.{class_name}")
                                        return icon_cls
                except Exception:
                    pass
